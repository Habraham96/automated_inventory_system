...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($email && $password) {
    // Query brm_users table for BRM authentication
    $stmt = $pdo->prepare('SELECT * FROM brm_users WHERE email = ?');
    $stmt->execute([$email]);
    $brm = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($brm && password_verify($password, $brm['password'])) {
      $_SESSION['brm_id'] = $brm['id'];
      $_SESSION['brm_name'] = $brm['fullname'] ?? $brm['name'];
      $_SESSION['brm_email'] = $brm['email'];
      $_SESSION['user_type'] = 'brm';
      $redirect = $_SESSION['redirect_after_login'] ?? 'brm/index.php';
      ...
      echo '<div id="successMessage" style="position:fixed;top:0;left:0;width:100vw;height:100vh;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.25);z-index:99999;">
        <div style="background:#fff;padding:40px 32px;border-radius:16px;box-shadow:0 4px 32px rgba(125,42,232,0.12);text-align:center;max-width:350px;width:90%;">
          ...
          <p style="color:#222;font-size:1.1rem;margin-bottom:16px;">Welcome, ' . htmlspecialchars($brm['fullname'] ?? $brm['name']) . '!</p>
        </div>
      </div>';
...
<head>
    ...
    <title>BRM Login | SalesPilot</title>
  ...
  <a href="index.php" class="nav_logo active"><img src="asset/images/salespilot%20logo2.png" alt="SalesPilot Logo" style="height:36px;display:block;object-fit:contain;"></a>
        <ul class="nav_items" style="width:100%;display:flex;justify-content:center;align-items:center;">
          <h2 style="margin:0 auto;text-align:center;font-size:1.7rem;font-weight:600;color:#7d2ae8;">Sales Pilot - BRM Login</h2>
        </ul>
      </nav>
    </header>
    <!-- Home -->
  ...
  <form action="" method="post" style="width:100%;">
          <h2 style="color: #0793fe;">BRM Login</h2>
          <p style="text-align:center;color:#6c757d;margin-bottom:20px;font-size:0.95rem;">Business Relationship Manager Portal</p>
