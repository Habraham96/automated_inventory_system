...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($email && $password) {
    // Query staff_table for staff authentication
    $stmt = $pdo->prepare('SELECT * FROM staff_table WHERE email = ?');
    $stmt->execute([$email]);
    ...
    if ($staff && password_verify($password, $staff['password_hash'])) {
      ...
      $redirect = $_SESSION['redirect_after_login'] ?? 'staff/index.php';
...
<head>
  ...
  <a href="index.php" class="nav_logo active"><img src="asset/images/salespilot%20logo2.png" alt="SalesPilot Logo" style="height:36px;display:block;object-fit:contain;"></a>
        <ul class="nav_items" style="width:100%;display:flex;justify-content:center;align-items:center;">
          <h2 style="margin:0 auto;text-align:center;font-size:1.7rem;font-weight:600;color:#7d2ae8;">Sales Pilot - Staff Login</h2>
        </ul>
      </nav>
    </header>
    <!-- Home -->
  ...
  <form action="" method="post" style="width:100%;">
          <h2 style="color: #0793fe;">Staff Login</h2>
    ...
    <div class="login_signup">Manager? <a href="log_in_manager.php">Login as Manager</a></div>
        </form>
      </div>
    </div>
  </div>
